...
ModuleInterface(NetworkStack)
    // parameters:
    Parameter(metric, ParType_String)
    Parameter(dumpRoutes, ParType_Numeric)
    Parameter(externalApplType, ParType_String)
    Parameter(externalGenerateTransport, ParType_Numeric)
    Parameter(callFinishExternalAppl, ParType_Numeric)
    Parameter(shell, ParType_String)
    Parameter(shell_result, ParType_String)
    // gates:
    Gate(netIn[], GateDir_Input)
    Gate(fromAppl[], GateDir_Input)
    Gate(netOut[], GateDir_Output)
    Gate(toAppl[], GateDir_Output)
EndInterface

Register_ModuleInterface(NetworkStack)

//// Sample code:
// class NetworkStack : public cSimpleModule
// {
//     Module_Class_Members(NetworkStack,cSimpleModule,16384)
