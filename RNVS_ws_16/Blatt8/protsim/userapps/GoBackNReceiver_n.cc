...
ModuleInterface(GoBackNReceiver)
    // parameters:
    Parameter(applAddr, ParType_Numeric)
    Parameter(ackSize, ParType_Numeric)
    Parameter(processing, ParType_Numeric)
    Parameter(receiveQueue, ParType_Numeric)
    Parameter(sendNack, ParType_Numeric)
    // gates:
    Gate(in, GateDir_Input)
    Gate(out, GateDir_Output)
EndInterface

Register_ModuleInterface(GoBackNReceiver)

//// Sample code:
// class GoBackNReceiver : public cSimpleModule
// {
//     Module_Class_Members(GoBackNReceiver,cSimpleModule,16384)
