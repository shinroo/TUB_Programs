...
bool MoveServer::movePlanning(lilac_fundamentals::MoveToPosition::Request &req, lilac_fundamentals::MoveToPosition::Response &res){
	...
	if(srvExecute.response.success){
		
		res.success = success;
		return success;
	} else{
		nodeFromDeletedEdge = getNodes(srvExecute.response.startrow, srvExecute.response.startcolumn);
		deletedEdgeNode = nodeFromDeletedEdge->deleteEdge(srvExecute.response.endrow, srvExecute.response.endcolumn);
		res.startrow = srvExecute.response.startrow;
		res.startcolumn = srvExecute.response.startcolumn;
		res.endrow = srvExecute.response.endrow;
		res.endcolumn = srvExecute.response.endcolumn;
		res.success = success;
		return success;
	}
