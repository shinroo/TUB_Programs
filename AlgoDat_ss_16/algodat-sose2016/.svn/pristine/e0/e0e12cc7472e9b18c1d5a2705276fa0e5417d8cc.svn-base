...
public class Matcher implements IMatcher {
	...
	protected double[][] initializeAccDistanceMatrix(ISignal signalX, ISignal wavfileY) {
		int numX = signalX.getNumFrames()+1;
		int numY = signalY.getNumFrames()+1;
		
		//declare matrix
		double[][] AccDistanceMatrix = new double[numX][numY];
		
		//init matrix
		for (int x = 0; x < numX; x++){
			for (int y = 0; y < numY; y++){
				if (x == 0 && y == 0){
					AccDistanceMatrix[x][y] = 0;
				}
				else if (x == 0 || y == 0){
					AccDistanceMatrix[x][y] = Double.POSITIVE_INFINITY;
				}
				else {
					AccDistanceMatrix[x][y] = 0;
				}
	...
	protected void computeAccDistanceMatrix(double [][] accDistance, ISignal signalX, ISignal signalY) {
		...
		for (int x = 0; x < signalX.getNumFrames(); x++) {
		    for (int y = 0; y < signalY.getNumFrames();y++) {
		        double d = Math.abs(signalX.getFrame(x) - signalY.getFrame(y));
		        accDistance[x+1][y+1] = Math.min(accDistance[x][y], Math.min(accDistance[x][y+1], accDistance[x+1][y])) + d;	        
		    }
		}
	...
	protected double computeDistance(double[][] accDistance) throws RuntimeException {
		...
		double d = accDistance[accDistance.length-1][accDistance[accDistance.length-1].length-1];
		return d;
	}
	...
	protected LinkedList<Pair<Integer,Integer>> computeMatchingPath(double[][] accDistance) throws RuntimeException {
		...
		LinkedList<Pair<Integer,Integer>> matchingPath = new LinkedList<Pair<Integer,Integer>>();
		
		//initialise vars
		int x = accDistance.length -1;
		int y = accDistance[x].length -1;
		...
		while ( !(x == 0) && !(y == 0) ){
			...
			matchingPath.addFirst(pair);
			
			//compute next values for x and y
			left = accDistance[x-1][y];
			top = accDistance[x][y-1];
			topLeft = accDistance[x-1][y-1];
			
			min = Math.min(left, Math.min(top, topLeft));
