...
public class DiGraph implements Graph {
	...
	public Graph toMinSpanTree() {
		...
		// TODO Homework 2.3
		//create graph to be returned and unionfind data structure
		UnionFindSet<Node> UFS = new UnionFindSet<Node>();
		DiGraph returnGraph = new DiGraph();
		...
		//work through priority queue and see which edges to add to returnGraph
		Edge temp;
		while (!edges.isEmpty()){
			temp = edges.poll();
			if (!(UFS.getRepresentative(temp.getStartnode()).equals(UFS.getRepresentative(temp.getEndnode())))){
				UFS.union(temp.getStartnode(), temp.getEndnode());
				returnGraph.addEdge(temp.getStartnode().id, temp.getEndnode().id, temp.getWeight());
			}
		}
