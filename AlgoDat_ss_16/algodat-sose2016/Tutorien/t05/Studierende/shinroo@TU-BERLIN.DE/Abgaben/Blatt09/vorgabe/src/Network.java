...
public class Network implements INetwork {
	...
	public List<Edge> findMinCut() {
		...
		//find reachable nodes in residual network
		LinkedList<Node> reachableNodes = new LinkedList<Node>();
		reachableNodes = residualGraph.findReachableNodes(source);
		
		//loop through all edges in reachable nodes, if the edge leads to
		//a node which is not in reachableNodes, add it to min cut
		for (Node currentNode: reachableNodes){
			for (Edge currentEdge: currentNode.getIncidentEdges()){
				if (!reachableNodes.contains(currentEdge.getEndnode())){
					if (currentEdge.capacity - currentEdge.currentFlow == 0)
						cutEdges.add(currentEdge);
				}
			}
		}
			...
			while (!queue.isEmpty()) {
				...
				for (Node neighbor : current.getSuccessorNodes()) {
					...
					if (neighbor.status == Node.WHITE) {
						...
						//check if the edge has a nonzero flow capacity, and if yes add it to the queue
						Edge currentEdge = getEdge(current.id, neighbor.id);
						if (currentEdge.capacity - currentEdge.currentFlow > 0)
