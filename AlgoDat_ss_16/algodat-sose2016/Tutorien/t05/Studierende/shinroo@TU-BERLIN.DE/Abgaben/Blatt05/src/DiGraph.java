...
public class DiGraph implements Graph {
	...
	public List<Node> breadthFirstSearch(Node startNode){
		...
		//handle first node
		startNode.status = startNode.GRAY;
		stopExecutionUntilSignal();
		toVisit.addAll(startNode.getWhiteSuccessors());
		startNode.status = startNode.BLACK;
		nodeList.add(startNode);
		Node temp = null;
		//breadth first walk through rest of reachable nodes
		while(!(toVisit.isEmpty())){
			temp = toVisit.getFirst();
			temp.status = temp.GRAY;
			stopExecutionUntilSignal();
			toVisit.poll();
			toVisit.addAll(temp.getWhiteSuccessors());
			temp.status = temp.BLACK;
			nodeList.add(temp);
			stopExecutionUntilSignal();
		}
		return nodeList;
	...
	public List<Node> depthFirstSearch(Node startNode){
		...
		while(!(toVisit.isEmpty())){
			...
			stopExecutionUntilSignal();
			toVisit.poll();
			LinkedList<Node> tempList = temp.getWhiteSuccessors();
			while(!(tempList.isEmpty())){
				toVisit.addFirst(tempList.getLast());
				tempList.removeLast();
			}
			temp.status = temp.BLACK;
			nodeList.add(temp);
